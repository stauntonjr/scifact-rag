# SciFact RAG technical reference

This document preserves the complete retrieval-strategy, command, result-schema, provenance, and
operator reference that previously lived in the root README. Start with the concise project
...
Retrieval defaults to `pooled-coref-interval-content-max-colbert`. It builds the established
six-generator pool and ranks candidates only by the maximum raw ColBERT score over bounded DP
content views. This requires the stored BM25, title, token-window, coreference-sentence, interval,
and DP representations plus the healthy ColBERT service. `bm25-token-window-rrf` remains the
fast/no-ColBERT fallback, and the former dedicated-title plus abstract-window default remains
selectable as `title-token-window-rrf`.

ADR-0029 selected the default after a fixed internal comparison reached nDCG@10 0.742493 versus
0.673519 for the BM25 fallback, with median latency 638.94 ms versus 78.70 ms. The evidence supports
the repository's retrieval-effectiveness choice but is not an unbiased generalization estimate.
Three abstract-only coreference strategies also remain selectable: strict proper-noun sentence
resolution, broader explicit-nominal sentence resolution, and maximum-score fusion across both.
Coreference is resolved over each abstract before sentence splitting. No abstract-derived
embedding channel contains the title; every channel aggregates its best representation score back
to one document result, preserving document IDs for citations.
...
[`retrieval-evaluation.md`](retrieval-evaluation.md). This comparison is
...
[`../reports/issue-6-retrieval-default-validation.md`](../reports/issue-6-retrieval-default-validation.md)
and [`../adr/0029-retrieval-default-selection.md`](../adr/0029-retrieval-default-selection.md).
Running the evaluator did not itself change the retrieval default; Issue #7 later applied its
accepted recommendation without rerunning or tuning the comparison.
...
coreference representation as well. `title-token-window-rrf` remains a selectable former default;
...
maximum-content scores without rescaling them. Content-max is now the default; multiview and raw
mean remain opt-in ablations.
...
[the retrieval leaderboard](../reports/scifact-retrieval-leaderboard.md) for all metrics and
provenance.
...
The current BM25-plus-abstract-token RRF reaches test nDCG 0.669962 and recall 0.819222. The former
dedicated-title plus abstract-token equal-RRF default reaches only nDCG 0.548652 and recall
0.705167. It remains selectable but is no longer the default.
...
See [the composition ADR](../adr/0013-scifact-rag-composition-and-runtime.md),
[the modular coreference ADR](../adr/0015-modular-coreference-retrieval.md),
[the keyword and RRF ADR](../adr/0016-postgresql-keyword-and-rrf.md),
[the VectorChord-BM25 ADR](../adr/0017-vectorchord-bm25.md),
[the cross-encoder reranking ADR](../adr/0018-msmarco-cross-encoder-reranking.md),
[the pooled-ranking ADR](../adr/0020-pooled-ranking-and-projection-alignment.md),
[the robust score-fusion ADR](../adr/0021-generic-feature-ranking-and-robust-score-fusion.md),
[the hosted ColBERT ADR](../adr/0022-hosted-colbert-late-interaction-scoring.md),
[the semantic-chunking ADR](../adr/0023-coreference-aware-semantic-chunking.md),
[the global coreference-interval ADR](../adr/0024-global-coreference-interval-packing.md),
[the expanded-pool reranker ADR](../adr/0025-expanded-coreference-interval-pool-rerankers.md),
[the title-separated/listwise ADR](../adr/0026-title-separated-pooling-and-listwise-reranking.md),
[the Procurement projection crosswalk](../architecture/retrieval-projection-crosswalk.md),
[the coreference dependency review](../research/scifact-rag-coreference-solutions.md), and
[the BM25 solution assessment](../research/scifact-rag-bm25.md),
