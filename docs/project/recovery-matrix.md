...
| Five failures stop the run | `R005` retry exhaustion | `python3 tools/recovery_scenarios.py --fixture R005` | State becomes `blocked`; attempt ID remains 5; no attempt 6 exists |
| Retry exhaustion can resume under reviewed new evidence | `R006` resumable handoff | `python3 tools/recovery_scenarios.py --fixture R006` | `human:IDENTITY` handoff starts revision 2, attempt 1 in `understand`; partial bytes remain |
